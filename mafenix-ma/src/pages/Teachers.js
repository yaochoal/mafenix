...
///////COMPONENTE QUE MUESTRA EL DOCENTE INDIVIDUAL//////////////////////
export class Teacher extends React.Component {
  constructor() {
		super()
		this.state = { data_a: [],loading: true }
  }
//////////////////////OBTENIENDO DATOS DEL DOCENTE//////////////////////////
  async componentWillMount(){
		await client.query({
			query: gql`
			query{
				teacherById(id:${this.props.id}){
				  id
				  name
				  description
				  teacher_has_courses {
    ...
    if (this.state.loading) {
      ...
      return (
        ...
        <Content  style={{width:"100%",height:"100%"}}>
          ...
          <Grafico recurso="teachers" post_id={this.state.data_a.id}/>
          <Title style={{color:'black'}}>Comentarios:</Title>
          <Comentarios recurso="teachers" post_id={this.state.data_a.id}/>
        </Content>
      </Container>  
      </View>
      );
...
///////// COMPONENTE QUE DA EL LISTADO DE LOS DOCENTES//////////////
    ...
    return (
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
            activeOpacity={0.8}
            onPress={()=> Actions.Teacher({nombre: item.name, id: item.id})}
            >
            <ListItem
              roundAvatar
              title={`${item.name} `}
